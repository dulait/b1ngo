...
public abstract class ApiController : ControllerBase
{
    protected PlayerIdentity Identity
        => (PlayerIdentity)HttpContext.Items[PlayerTokenAuthFilter.PlayerIdentityKey]!;

    protected SquareMarkedBy CallerMarkedBy
        => (SquareMarkedBy)HttpContext.Items[PlayerTokenAuthFilter.MarkedByKey]!;

    protected async Task<IActionResult> Send<TCommand, TResponse>(
        ICommandHandler<TCommand, TResponse> handler,
        TCommand command,
        CancellationToken ct,
        Action<TResponse>? onSuccess = null) where TCommand : ICommand<TResponse>
    {
    ...
    protected async Task<IActionResult> Send<TQuery, TResponse>(
        ...
        TQuery query,
        CancellationToken ct) where TQuery : IQuery<TResponse>
    {
    ...
    protected IActionResult ToActionResult(Error error)
        => error.Type switch
        {
            ErrorType.Validation => BadRequest(new { error.Code, error.Message, error.Details }),
