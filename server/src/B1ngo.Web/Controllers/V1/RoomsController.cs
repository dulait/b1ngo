...
public class RoomsController(
    ...
    IQueryHandler<GetRoomStateQuery, GetRoomStateResponse> getRoomStateHandler) : ApiController
{
    [HttpPost]
    public async Task<IActionResult> CreateRoom(
        [FromBody] CreateRoomCommand command, CancellationToken ct)
        => await Send(createRoomHandler, command, ct,
            response => SetPlayerTokenCookie(response.PlayerToken));

    [HttpPost("join")]
    public async Task<IActionResult> JoinRoom(
        [FromBody] JoinRoomCommand command, CancellationToken ct)
        => await Send(joinRoomHandler, command, ct,
            response => SetPlayerTokenCookie(response.PlayerToken));
    ...
    public async Task<IActionResult> EditSquare(
        Guid roomId, int row, int column,
        [FromBody] EditSquareRequest request, CancellationToken ct)
        => await Send(editSquareHandler,
            new EditSquareCommand(Identity.RoomId, Identity.PlayerId, row, column, request.DisplayText), ct);
    ...
    public async Task<IActionResult> MarkSquare(
        Guid roomId, Guid playerId, int row, int column, CancellationToken ct)
        => await Send(markSquareHandler,
            new MarkSquareCommand(roomId, playerId, row, column, CallerMarkedBy), ct);
    ...
    public async Task<IActionResult> UnmarkSquare(
        Guid roomId, Guid playerId, int row, int column, CancellationToken ct)
        => await Send(unmarkSquareHandler,
            new UnmarkSquareCommand(roomId, playerId, row, column), ct);

    private void SetPlayerTokenCookie(Guid playerToken)
    {
        Response.Cookies.Append("PlayerToken", playerToken.ToString(), new CookieOptions
        {
            HttpOnly = true,
            Secure = true,
